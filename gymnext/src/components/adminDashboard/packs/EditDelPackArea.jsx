...
// Delete pack func
    const deletePack = (pack) => {
     ...
     alert('Pack.id - editdelPack-area - line 101:' + pack.image);
   
// function to delete the image in backend     
 const ImagePack = pack?.image;  
 const Image = new FormData();
       Image.append("image", ImagePack);
...
DeleteImage(
  ...
  // you get the result from packApi - function useDeletePackImageMutation(), mutation for post request
     ...
     alert('Pack.id - editdelPack-area - line 118:' + pack.id);
    //DeletePackService(pack._id);
    RDdeletePack(pack.id).then((result) => {
      if (result?.error) {
