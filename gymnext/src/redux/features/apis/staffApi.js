...
export const staffApi = apiSlice.injectEndpoints(
  ...
  endpoints: (builder) => ({
    ...
    registerStaff: builder.mutation({
      query: (data) => ({
        ...
        url: `${BACKEND_BASE_URLBE}`+'api/trainers/create', //Development mode
        method: "POST",
        body: data,
      }),
    }),
    ...
    deleteStaffImage: builder.mutation({
      query: (data) => ({   
        //url: "https://shofy-backend.vercel.app/api/user/signup",
        //url: "http://localhost:7000/api/user/signup",
        url: `${BACKEND_BASE_URLBE}`+'api/trainers/DeleteImage', //Development mode
        method: "POST",
        body: data,
      }),
    }),
    ...
    getStaff: builder.query({
     // query: (id) => `https://shofy-backend.vercel.app/api/product/single-product/${id}`,
      query: (id) => `${BACKEND_BASE_URLBE}api/trainers/get-single-trainer/${id}`,
      providesTags: (result, error, arg) => [{id: arg }],
    }),
    // get all Staffs

   getAllStaffs:builder.query({   
    query: () => `${BACKEND_BASE_URLBE}`+'api/trainers/listAll',
   }),
    ...
    updateStaff: builder.mutation({
      query: ({ selected, data }) => ({
        ...
        url: `${BACKEND_BASE_URLBE}api/trainers/update-trainer/${selected.id}`,
        method: "PUT",
        body: data,
      }),
    }),
 // delete Product endpoint
     deleteStaff: builder.mutation({
      query: ( id ) => ({          
        // url: "https://shofy-backend.vercel.app/api/category/add",
        url: `${BACKEND_BASE_URLBE}api/trainers/delete-trainer/${id}`,
        method: "DELETE",
      }),
    }),// End of delete Staff
